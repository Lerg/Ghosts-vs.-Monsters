...
function scene:create(event)
	...
	local backgroundFilename1 = 'images/altbackground1.png'
	local backgroundFilename2 = 'images/altbackground2.png'
	...
	local callGameOver = function( isWin )
		...
		-- SHADE
		local shadeRect = display.newRect(_CX, _CY, 480, 320 )
		...
		-- RESTART BUTTON
		local onRestartTouch = function()
			sounds.play('tap')
			composer.gotoScene('scenes.reloadgame', {params = levelId})
		end
		...
		local restartBtn = widget.newButton({
			...
			width = 60, height = 60,
			x = menuBtn.x + 72, y = 186,
			onRelease = onRestartTouch
		})

		restartBtn.alpha = 0

		-- NEXT BUTTON
		local onNextTouch = function()
			sounds.play('tap')
			composer.gotoScene('scenes.reloadgame', {params = levelId + 1})
		end

		local nextBtn = widget.newButton({
			defaultFile = 'images/buttons/nextlevel.png',
			overFile = 'images/buttons/nextlevel-over.png',
			width = 60, height = 60,
			x = restartBtn.x + 72, y = 186,
			onRelease = onNextTouch
		})

		nextBtn.alpha = 0
		if isWin ~= 'yes' then nextBtn.isVisible = false; end

		if levelId == composer.getVariable('levelCount') then
			nextBtn.isVisible = false
		end

		local onFBTouch = function()
			sounds.play('tap')
		end

		fbBtn = widget.newButton({
			defaultFile = 'images/buttons/facebook.png',
			overFile = 'images/buttons/facebook-over.png',
			width = 302, height = 40,
			x = 240, y = 220,
			onRelease = onFBTouch
		})
		...
		if gameScore > bestScore then
			bestScore = gameScore
			local bestScoreFilename = levelId .. '.data'
		...
		bestScoreText:setTextColor( 228 / 255, 228 / 255, 228/ 255, 1 )	--> white
	...
	local drawBackground = function()
		...
		-- TREES
		local treesLeft = display.newImageRect( 'images/trees-left.png', 480, 320 )
		treesLeft.x = 240; treesLeft.y = 160

		local treesRight = display.newImageRect( 'images/trees-right.png', 480, 320 )
	...
	local drawHUD = function()
		...
		scoreText:setFillColor( 1, 1, 1, 1 )	--> white
		...
		continueText:setFillColor( 249/ 255, 203/ 255, 64/ 255, 1 )
		...
		local onPauseRelease = function()
				...
				if gameIsActive then
					...
					if not shadeRect then
						shadeRect = display.newRect(_CX, _CY, 480, 320 )
						shadeRect:setFillColor( 0, 0, 0, 1 )
						hudGroup:insert( shadeRect )
					end
					shadeRect.alpha = 0.5
		...
		pauseMenuBtn = widget.newButton({
			defaultFile = 'images/buttons/pausemenu.png',
			overFile = 'images/buttons/pausemenu-over.png',
			width = 44, height = 44,
			x = 38, y = 288,
			onRelease = onMenuPauseTouch
		})
	...
	local createGround = function()
		...
		groundObject1 = display.newImageRect( 'images/ground1.png', 480, 76 )
		groundObject1.anchorX, groundObject1.anchorY = 0, 1
		groundObject1.x = 0; groundObject1.y = 320

		groundObject2 = display.newImageRect( 'images/ground2.png', 480, 76 )
		groundObject2.anchorX, groundObject2.anchorY = 0, 1
	...
	local createGhost = function()
		...
		ghostObject = display.newSprite(graphics.newImageSheet('images/ghost1.png',
	        {width = 26,
	        height = 26,
	        numFrames = 2,
	        sheetContentWidth = 52, sheetContentHeight = 26}),
	        {name = 'normal',
	        start = 1,
	        count = 2})
	    ghostObject:setSequence('normal')
		...
		physics.addBody( ghostObject, 'static', { density=10.0, bounce=0.4, friction=0.15, radius=ghostObject.radius } )
		ghostObject.rotation = 0
		ghostObject:setFrame( 1 )
	...
	local onMonsterPostCollision = function( self, event )
		if event.force > 1.5 and self.isHit == false then
			sounds.play('monsterPoof')

			self.isHit = true
			print( 'Monster destroyed! Force: ' .. event.force )
			self.isVisible = false
			timer.performWithDelay(1, function(self)
		...
		if gameIsActive then
			...
			elseif event.phase == 'ended' and ghostObject.isReady then
				...
				local flingNow = function()
					...
					local y = event.y
					local forceValue = 30
					local xForce = (-1 * (x - ghostObject.x)) * forceValue
					local yForce = (-1 * (y - ghostObject.y)) * forceValue

					sounds.play('wee')

					ghostObject:setFrame( 2 )
	...
	local createLevel = function()
		...
		levelGroup, monsterCount = require('levels.' .. levelId).build(onMonsterPostCollision)
	...
	local gameInit = function()
		...
		local bestScoreFilename = levelId .. '.data'
		local loadedBestScore = loadValue( bestScoreFilename )
